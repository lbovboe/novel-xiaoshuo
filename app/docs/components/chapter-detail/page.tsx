...
export default function ComponentsPage() {
    ...
    sections: {
      ...
      props: {
        ...
        items: [
          ...
          language === 'en' ? (
            <>
              <strong>nextChapterExists</strong>: <code>boolean</code> — Whether a next chapter exists for navigation
              and auto-next.
            </>
          ) : (
            <>
          ...
          language === 'en' ? (
            <>
              <strong>chapterIndex</strong>: <code>number</code> — The index of the current chapter (for navigation and
              storage).
            </>
          ) : (
            <>
      ...
      features: {
        ...
        items: [
          language === 'en' ? (
            <>
              <strong>Content Formatting</strong>: Splits chapter content into paragraphs and applies user-selected font
  ...
  return paragraphs.map((paragraph, index) => (
    <p key={index} className={"mb-4 " + fontSizeClass + " leading-relaxed text-light-text-primary dark:text-dark-text-primary"}>
      {convertText(paragraph)}
          ...
          language === 'en' ? (
            <>
              <strong>Navigation</strong>: Provides previous/next chapter navigation and a link back to the chapter
              list.
              <DocCodeBlock
  ...
  {nextChapterExists && (
    <Link href={\`/book/\${bookId}/chapter/\${chapterIndex + 1}\`}>{convertText('下一章 →')}</Link>
            ...
            <>
              <strong>章节导航</strong>: 提供上一章/下一章导航和返回章节列表的链接。
              <DocCodeBlock
                code={`<div ref={footerRef} className="mt-8 flex justify-between">
  {prevChapterExists && (
    <Link href={\`/book/\${bookId}/chapter/\${chapterIndex - 1}\`}>{convertText('← 上一章')}</Link>
  ...
  try {
    ...
    // handle error
            ...
            <>
              <strong>进度持久化</strong>: 使用 <code>localStorage</code>{' '}
              保存用户当前章节和阅读进度，并包含存储空间管理逻辑。
              <DocCodeBlock
                code={`useEffect(() => {
  try {
    const storageKey = \`book_data_\${bookId}\`;
    ...
    // handle error
          ...
          language === 'en' ? (
            <>
              <strong>Auto-Next</strong>: Optionally auto-navigates to the next chapter when the user scrolls to the
              bottom, with a visible countdown.
              <DocCodeBlock
                code={`useEffect(() => {
  ...
  return () => { if (footerRef.current) observer.unobserve(footerRef.current); };
            ...
            <>
              <strong>自动翻页</strong>: 当用户滚动到底部时可自动跳转到下一章，并显示倒计时提示。
              <DocCodeBlock
                code={`useEffect(() => {
  if (!autoNext || !nextChapterExists) return;
  const observer = new IntersectionObserver((entries) => {
  ...
  return () => { if (footerRef.current) observer.unobserve(footerRef.current); };
          ...
          language === 'en' ? (
            <>
              <strong>Responsive & Animated UI</strong>: Uses animated transitions and adapts to light/dark themes for a
  ...
  <div className="container ...">
    ...
    {/* Navigation and auto-next UI ... */}
            ...
            <>
              <strong>响应式与动画界面</strong>: 使用动画过渡并适配明暗主题，提升阅读体验。
              <DocCodeBlock
                code={`return (
  <div className="container ...">
    <NavBar />
